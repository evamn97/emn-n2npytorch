...
#SBATCH -J 02-n2npt-test1           				    # Job name
...
pdm run python src/test.py -d new_test_data/512/ -n ${noise} -p ${param} --show-output ${show} -c ${crop} --output ${results} --load-ckpt "${ckpt_name}" --cuda
